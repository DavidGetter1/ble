...
// here we handle data from that we got via scan
static bool eir_found(struct bt_data *data, void *user_data)
{
	...
	case BT_DATA_UUID16_ALL:
		...
		for (i = 0; i < data->data_len; i += sizeof(uint16_t)) {
			...
			printk("Asking to create connection ble");
...
// hier wurde ein device gefunden per scan
			 ...
			 struct net_buf_simple *ad)
{
	...
	if (type == BT_HCI_ADV_IND || type == BT_HCI_ADV_DIRECT_IND) {
		// hier greifen wir auf die daten die wir im scan bekommen haben zurück
...
// hier geht das scannen los, device_found ist der cb
